...
class listener(StreamListener):
    ...
    def __init__(self, api=None):
        self.num_tweets = 0
        self.num_files = 1
        self.filename = 'tweets_USA_elections_2020_10K_1.json'
            ...
            with open(self.filename, 'a') as tweet_file:
                ...
                if self.num_tweets >= max_tweets:
                    ...
                    if self.num_files >= max_files:
                        ...
                        self.num_files += 1
                        self.filename = 'tweets_USA_elections_2020_10K_'+str(self.num_files)+'.json'
                        self.num_tweets = 0
                        self.pbar = ProgressBar(widgets=[Percentage(), Bar()], maxval=max_tweets).start()
                        print(self.filename) 
                else:
...
twitterStream.filter(track=["biden","Biden","Trump","trump","vote","voting","elections"])
